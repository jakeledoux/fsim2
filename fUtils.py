import random, mimesis, colorama, os
...
def printd(intext,players=[]):
	while "NAME" in intext:
		...
		intext = intext[:name_index]+Style.BRIGHT+Fore.YELLOW+players[int(intext[name_index+4])-1].name+Style.RESET_ALL+intext[name_index+5:]

	for PRONOUN in pronoun_list:
		while PRONOUN in intext:
			pronoun_index = intext.index(PRONOUN)
			if pronoun_index == 0:
				intext = intext[:pronoun_index]+pronoun_list[PRONOUN][players[int(intext[pronoun_index+len(PRONOUN)])-1].gender].title()+intext[pronoun_index+1+len(PRONOUN):]
			else:
				intext = intext[:pronoun_index]+pronoun_list[PRONOUN][players[int(intext[pronoun_index+len(PRONOUN)])-1].gender]+intext[pronoun_index+1+len(PRONOUN):]
...
class gen:
	def location(type):
		if type == "grocery_store":
			return random.choice(['Whole Foods Market','Kroger','Costco','Walmart'])
		elif type == "gas_station":
			return random.choice(['Exxon','7-Eleven','Mobil','Sunoco'])
		elif type == "farm":
			prefix = random.choice(["","Old Man","Lucky","Family"])
			name = mimesis.Person().last_name(gender="male")
			apostrophe = random.choice([True, False])
			name += "'s" if apostrophe else ""
			suffix = random.choice(['Farm','Animal Farm','Vegetable Farm','Ranch','Natural Farm'])
			suffix += "s" if suffix[-4:] == "Farm" and not apostrophe else ""
			return " ".join([prefix,name,suffix]).strip()
		elif type == "apartment_building":
			return mimesis.Address().street_name()+" "+random.choice(["Apartments","Condos"])
		elif type == "house":
			return mimesis.Person().last_name()+" Residence"
		elif type == "street":
			return mimesis.Address().street_name()+" "+random.choice(['Street','Road','Street','Road','Turnpike','Way','Lane','Parkway'])
	def biome(type):
		if type == "urban":
			...
			return mimesis.Address().city()+" "+random.choice(['Swamp','Mountain','Valley','Plains','Countryside'])
...
class NPC:
	...
	def __init__(self, name, gender):
		...
		self.mean = random.choice([False,False,True]) # 2:1 odds of being nice. If mean, player rolls twice on evil actions.
	...
	def eat(self):
		...
		if food_item != None: # If food item found
			self.hunger = max(self.hunger-food_item.effectiveness, 0) # Replenish hunger, but don't let it become negative.
	...
	def drink(self):
		...
		for drink in water: # Drink until no longer thirsty or run out of water, starting with container with fewest fl oz.
